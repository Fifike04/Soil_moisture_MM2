...
st.title("Step 9 — Experiment runner (reproducible run pack) — DISK ONLY from outputs/")
...
# -----------------------------
# Load Step2 artifacts from disk
# -----------------------------
...
st.subheader("Loaded sequences from outputs/step2")
st.write({
    "X": tuple(X.shape),
    "y": tuple(y.shape),
    "meta_rows": int(len(meta_df)),
    "targets": target_names[:10] + (["..."] if len(target_names) > 10 else [])
})
...
if st.button("Run (train + evaluate + export run pack)"):
    ...
    rlrop = callbacks.ReduceLROnPlateau(
        monitor="val_loss", factor=0.5, patience=max(2, int(patience // 2)), min_lr=1e-6
    )
